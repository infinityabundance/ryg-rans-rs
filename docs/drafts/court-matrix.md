...
All verdicts below are "PASS" based on Rust self-tests only.
No compiled oracle comparison has been sealed for any court.
(Each PASS is denoted `PASS†` — see footnote.)
...
† *Rust self-test only. No compiled oracle comparison has been sealed.*
