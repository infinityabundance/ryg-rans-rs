...
| L18-C | LOW | The `bench` subcommand of the CLI and the Docker matrix were not part of the L.18 run | OPEN | Docker matrix re-run at the final implementation commit (earlier runs exposed two defects now fixed: CLI 8-way code unbuildable under `--no-default-features` (MSRV), and the comparative FFI forcing bindgen/libclang into musl/MSRV builds — see L18-D2/L18-D3); CLI `bench` subcommand is wired and integration-tested (L.15) and covered by the workspace test suite in the seal |
