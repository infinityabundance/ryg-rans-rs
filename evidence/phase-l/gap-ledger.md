...
| L22-D | MEDIUM | Release v0.4.0 (pre-1.0 minor per semver-checks: the L8-REOPEN fix changed public `ValidatedModelArtifacts.freqs` to `Arc<Vec<u32>>` and added `packed_table`) | RESOLVED | `ee8fb0e` (version decision) — full evidence regenerated at the 0.4.0 commit (run `phase-l-20260802e`, 800×100, 158 behavioural receipts, Docker matrix `ci-20260802-ee8fb0e` 11/11); all seven crates published at 0.4.0 in dependency order from the sealed commit; annotated tag `v0.4.0` at the sealed evidence commit; full seal gate green |
