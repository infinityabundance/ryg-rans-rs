...
| `cargo xtask workload <fetch|derive|synthetic-cache-stress|synthetic-cache-soak|stress-public|soak-public|policy-sim> [public-rans-v1]` | ✅ implemented | Public-corpus workload tooling — see [`workloads/public-rans-v1/README.md`](../workloads/public-rans-v1/README.md). `stress`/`soak` are aliases of the synthetic runners (labeled `synthetic-cache-stress-v1`); `stress-public`/`soak-public` execute the derived manifest schedule on the hash-verified corpus bytes (`--schedule` selects smoke/1g/mixed-16g/stress-64g); `policy-sim` runs the offline FIFO/LRU shadow simulation. |
...
- **Package version**: 0.1.0 (workspace crates are 0.4.0). Workspace-internal;
  never published.
